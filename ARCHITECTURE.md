...
- **Управление битрейтом (слайс 2.3):** контроллер на хосте (`host/src/adapt`), тик раз в
  секунду, входы — **REMB** от браузера (оценка его receive-side GCC; сигнал перегрузки
  только когда REMB ниже фактически отправленного битрейта, т.к. receive-side оценка
  ограничена ~1.5× принятого), **доля потерь из RTCP RR** (> 10 % — срез, < 2 % — рост
  пробингом +10 %/с) и **задержка захват→кодер** (кодер не успевает — ступень fps вниз).
