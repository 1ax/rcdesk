...
- ✅ **Слайс 2.4 мультимонитор — в `main`, четыре коммита** (см. запись 2.4): `9f37676` контракт
  ...
  сессию. На стенде и на реальном захвате Mac не проверено — за владельцем (у него один монитор: селектор
  должен быть скрыт, мышь работать как раньше).
...
- **Проверка 2.4 вживую** (после push и зелёного Deploy): стенд Win10 — `list-displays` печатает
  `id title WxH@x,y primary`, `serve` без флагов логирует `capturing display`, селектора в клиенте нет
  (один монитор), мышь попадает куда надо (новый путь `SendInput`+`VIRTUALDESK`); Mac с реальным захватом —
  то же, мышь по `CGDisplayBounds`.
