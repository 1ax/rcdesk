...
**Состояние на 2026-09-11 (конец первого чата, слайсы 0.1–1.6):**
- ✅ **Фаза 1 закрыта и задеплоена.** https://rcdesk.app — веб-клиент; сигнальный сервер и
  coturn в Docker на VPS владельца (пользователь `rcdesk`, `~/app`, порт 127.0.0.1:8100,
  nginx FastPanel проксирует `/ws` и `/healthz`). Push в `main` → CI (3 ОС) → образ
  `ghcr.io/1ax/rcdesk-server` → статика в docroot → `docker compose up` (`.github/workflows/deploy.yml`).
- ✅ Проверено вживую на проде (Chrome 153, реальный экран 1920x1080): connect ~1 с, 29 fps при
  движении, H264, rtt 1 мс, app-RTT 1 мс, форма курсора с хоста применяется; TURN выдаёт
  relay-кандидаты (проверено trickle-ICE с REST-кредами). Safari — проверен владельцем на
  локальном стенде (видео).
- ▶️ **Следующий слайс — 2.1: сборка и запуск хоста на Windows 10 x64** (физический ПК
  владельца). Затем 2.2 аппаратные кодеры, 2.3 адаптивный битрейт. Новый чат — на слайс.
- Как запустить: `docs/dev-run.md` (локально) и `infra/README.md` (прод). Хост к проду:
  `cargo run -p rcdesk-host -- serve --server wss://rcdesk.app/ws` (+ `--no-input` без
  «Универсального доступа»).
- Окружение владельца: Apple M4 / macOS 26.6 / Chrome 153 / Safari 26.6, Rust 1.98 в
  `~/.cargo/bin`, разрешение «Запись экрана» выдано терминалу; «Универсальный доступ» — нет.
- Правила процесса, добавленные по ходу: пока работает исполнитель — только явные пути в
  `git add`; живые проверки архитектора — из worktree на другом порту с переименованными
  бинарниками (исполнитель убивает процессы по маске); команды владельцу для сервера — через
  `pbcopy`, многострочная вставка в его терминал ломается.

**За владельцем:**
- Выдать «Универсальный доступ» приложению терминала и проверить ввод (мышь/клавиатура)
  с другого устройства или окна.
...
- ⚪ D9: задержка glass-to-glass численно не измерена (цель ≤ 80 мс по LAN); измерить по методике ARCHITECTURE §12 в начале фазы 2.
- ⚪ D10: TURN без TLS (5349): сертификаты LE в `/var/www/httpd-cert/` пользователю `rcdesk` не читаются; вариант — попросить владельца дать группе `rcdesk` право чтения или использовать coturn с собственным сертификатом.
- ⚪ D11: пакет `ghcr.io/1ax/rcdesk-server` — визибилити не проверена через API (нет scope read:packages у gh), но `docker compose pull` на сервере без логина прошёл → публичный или привязан к публичному репо.
- ⚪ D12: шум логов хоста (`rtc_dtls` WARN «Unsupported Extension», mDNS/STUN таймауты) — уровни логов в 3.5.
- ⚪ D13: `WinCursorSource` и Windows-ветка `scap`/`enigo` проверены только сборкой в CI — живая проверка в 2.1.
