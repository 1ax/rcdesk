...
**Состояние на 2026-09-21 (седьмой чат: слайс 3.1 ЗАКРЫТ — проверен владельцем вместе с фиксами):**
...
**За владельцем:**
- **Живая проверка 3.1 после деплоя** (первая: ни один под-шаг 3.1 на живой системе не запускался).
  1. Открыть https://rcdesk.app, войти по PIN на стенд Win10 — устройство должно появиться в «Мои компьютеры».
  2. Закрыть сеанс, подключиться к нему же из списка (без PIN). Переименовать, убедиться, что имя держится
     после перезагрузки страницы; удалить и привязать заново по PIN.
  3. Перезапустить агент на стенде: PIN новый, устройство в списке — то же самое, подключение из списка работает.
  4. Необязательно, проверка «сервер потерял базу»: на VPS `docker compose -f ~/app/docker-compose.prod.yml down -v`
     и `up -d` — хост должен сам перерегистрироваться (в логе `the server does not know this device`),
     список в браузере — опустеть.
