...
**Состояние на 2026-09-14 (конец третьего чата, слайс 2.2 закрыт, проверен на стенде):**
...
- ✅ **Windows-часть 2.2 проверена в CI и вживую на стенде** (2026-09-14, см. запись 2.2f): программный
  «H264 Encoder MFT» Microsoft обходит openh264 по задержке (27 мс против 67–78), fps под видео (20–23
  против 13–17) и резкости текста. **Дефолты (`2.2f`):** `auto` на Windows = любой MFT (аппаратный, затем
  программный), openh264 только как откат; openh264 с потолком QP 30 по умолчанию (D15 закрыт).
  Push `main` делает владелец сам; последний зелёный Deploy — `34777760519` (до 2.2f).
- ▶️ **Следующий слайс — 2.3 адаптивный битрейт/fps** по RTCP RR и TWCC (ARCHITECTURE §5, §13): на стенде
  под видео openh264 с потолком копит jitter-буфер до 80 мс (кодер не успевает за 1080p), нужна адаптация
  fps/битрейта/разрешения. Новый чат — на слайс.
...
**За владельцем:**
- Запушить `main` (2.2f: дефолты кодеров + доки), затем на стенде обычный `serve` без флагов должен
  логировать `media foundation encoder name=H264 Encoder MFT hardware=false`.
