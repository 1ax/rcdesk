...
- ✅ **D25 закрыт (2026-09-14):** `name: rcdesk` в `infra/docker-compose.prod.yml`, на VPS rcdesk пересоздан как
  проект `rcdesk` (`rcdesk-server-1`, `rcdesk-coturn-1`, сеть `rcdesk_default`), crewtally поднята в своём проекте
  `app` из её каталога — crewtally.app и `/api/health` 200, БД на head. Заодно починен redis glitchtip
  (крэш-цикл с 2026-09-10, битый incr-AOF; `redis-check-aof --fix`, бэкап `appendonlydir.bak-20260914-212802`
  внутри тома). Теперь на VPS три compose-проекта: `app` (crewtally), `glitchtip`, `rcdesk`.
- ▶️ **Следующее:** 2.4e монитор трафика (D22; ждёт развилок владельца — канал уведомлений и действие на
