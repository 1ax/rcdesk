...
**Состояние на 2026-09-13 (конец второго чата, слайс 2.1 закрыт):**
- ✅ **Фаза 1 в проде** (https://rcdesk.app, VPS владельца, деплой по push в `main`, см. голову 2026-09-11
  ниже по истории). ✅ **Слайс 2.1 закрыт:** хост живёт на Windows 10 стенде владельца, ввод и курсор
  проверены вживую с Mac (Chrome). Артефакты хоста собирает Deploy (`rcdesk-host-windows-x64`,
  `rcdesk-host-macos-arm64`, страница запуска → Artifacts). Инструкция: `docs/host-windows.md`.
- ⚠️ **Стенд владельца не тянет WGC:** ATI Radeon HD 4600 (Direct3D 10.1, легаси-драйвер WDDM 1.1),
  Ivy Bridge 4 потока. Хост сам уходит на **GDI-захват** (`--capture auto`, пробное `D3D11CreateDevice`),
  на стенде это 12–14 fps при 1080p и ~120 мс jitter-буфера под движением. Аппаратного кодера у
  этой карты нет — 2.2 на стенде проверить нельзя; у Ivy Bridge есть iGPU с Quick Sync, если владелец
  переключит монитор на встроенную графику (D14).
- ▶️ **Следующий слайс — 2.2 аппаратные кодеры** (VideoToolbox на Mac проверяем локально; Media Foundation —
  только сборкой в CI, пока нет стенда с D3D 11). Перед ним — решить развилку D15 (резкость/битрейт
  openh264): это видно пользователю уже сейчас. Новый чат — на слайс.
...
- Окружение владельца: Apple M4 / macOS 26.6 / Chrome 153 / Safari 26.6, Rust 1.98 в `~/.cargo/bin`;
  Win10 Pro 22H2 x64 стенд, exe в `d:\rc\`, Notepad++ и Yandex Browser на стенде.
- Правила процесса, добавленные в этом чате: живые проверки на стенде владельца — только мышь и
  чтение экрана, **никакого ввода с клавиатуры в неизвестное окно** (Notepad++ восстановил сессию с
  выделенным `.env`, и первый же символ его затёр — файл спасён отменой); автоматизация Chrome
  архитектора не шлёт отдельные нажатия модификаторов — Shift/Ctrl/Win проверяет владелец;
  `bench` на стенде не гонять параллельно с `serve` (делят CPU); один push в `main` = один запуск
  Deploy (CI отдельно только для веток и PR).

**За владельцем:**
- В FastPanel включить TLS 1.3 для nginx (сейчас только 1.2; хосту это уже не мешает).
- Развилка D14: монитор на встроенную графику Ivy Bridge (даст WGC + Quick Sync для 2.2) или
  оставить как есть.

**Открытые долги:**
- ⚪ D1: LICENSE для публичного репо не выбран (развилка владельцу: MIT / Apache-2.0 / без лицензии).
- ⚪ D3: `scap` 0.0.8: `Capturer` не `Send` (обёрнут `unsafe impl Send`), на Windows `start_capture` делает
  `unwrap()` на ошибке D3D (обходим пробным устройством), `DrawBorderSettings::Default` не настраивается.
  Переход на прямой `windows-capture` 2.x / `screencapturekit` — вместе с D5.
- ⚪ D4: openh264 с `skip_frames(false)` не держит потолок битрейта на сложном контенте; адаптация — 2.3.
- ⚪ D5: `windows-capture` запинен на =1.4.4 из-за scap 0.0.8; снять пин при переходе на прямой 2.x (с D3).
- ⚪ D6: хост не переподключается к сигнальному серверу при обрыве WS (процесс завершается) — 3.5.
- ⚪ D7: захват берёт первый `Target::Display` (GDI — первичный монитор), мышь масштабируется по
  `main_display()` — мультимонитор в 2.4.
- ⚪ D9: задержка glass-to-glass численно не измерена (цель ≤ 80 мс по LAN); измерить по методике
  ARCHITECTURE §12. Косвенно: `app 1–5 ms`, `jb 0 ms` в покое / ~120 мс под движением на GDI-стенде.
- ⚪ D10: TURN без TLS (5349): сертификаты LE в `/var/www/httpd-cert/` пользователю `rcdesk` не читаются.
- ⚪ D11: визибилити пакета `ghcr.io/1ax/rcdesk-server` не проверена через API; `pull` без логина проходит.
- ⚪ D12: шум логов хоста (`rtc_dtls` WARN «Unsupported Extension», mDNS/STUN таймауты) — уровни логов в 3.5.
- ⚪ D14: **GDI-захват медленный:** на стенде 12–14 fps при 1080p (BitBlt+сравнение ~70 мс), CPU заметно
  выше WGC; на этом железе иначе нельзя. Варианты: захват меньшей области/даунскейл, dirty-rect по
  сравнению строк, монитор на iGPU (WGC). Замерить CPU хоста в Диспетчере задач (не снято).
- ⚪ D15: **Резкость/битрейт:** openh264 отдаёт 0.35–1.1 Мбит/с при целевых 6, ключевые кадры ~120 КБ на
  1080p, картинка заметно размытая (текст в Notepad++ читается с трудом). Эксперименты архитектора на
  Mac bench (skip_frames(true), RC Quality, CameraVideoRealTime, wall-clock отметки) размер кадров не
  изменили (2.5–3.6 КБ). Следующие проверки: `QpRange` (ограничить max QP), `Complexity`, RC_OFF с
  фиксированным QP, аппаратный кодер 2.2. Развилка перед 2.2.
- ⚪ D16: WGC на Windows 10 рисует жёлтую рамку; `IsBorderRequired` появился в сборке 20348 (Windows 11),
  scap 0.0.8 не даёт настроить. На GDI рамки нет. Закрывается с D3/D5 на Windows 11.
- ✅ D13 закрыт: Windows-ветки (`WinCursorSource`, `enigo` мышь/колесо, keymap с E0, `SendInput`)
  проверены вживую в 2.1.
