...
**Состояние на 2026-09-18 (шестой чат: слайс 2.5, под-шаг 2.5a закрыт):**
- ✅ **2.5a — D26 закрыт** (`509cb6b`, см. запись 2.5a): без инжектора ввода (нет «Универсального доступа»
  или `--no-input`) сеанс живёт в режиме просмотра, хост шлёт `ControlMessage::InputStatus`, клиент не цепляет
  ввод и показывает `View only: <причина>`; при провале старта сессии хост шлёт клиенту `Bye`.
- ▶️ **Следующее: 2.5b+ буфер обмена** — разведка и план под-шагов; развилки: поллинг буфера хоста или
  события, лимит размера текста, защита от эха при двусторонней синхронизации.
...
- Mac с реальным захватом после 2.4 — проверен частично (2026-09-15): `list-displays` отдаёт настоящий
  ...
  не дошёл из-за D26 (нет «Универсального доступа» у родительского приложения). D26 закрыт в 2.5a: теперь
  сеанс покажет картинку с плашкой `View only`; мышь по `CGDisplayBounds` — после выдачи «Универсального
  доступа» приложению, из которого запускается хост.
...
### 2.5a Режим просмотра без разрешения на ввод, D26 (2026-09-18)
