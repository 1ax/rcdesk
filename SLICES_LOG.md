...
**Состояние на 2026-09-22 (девятый чат: 3.5g панель сеанса — в `main`, ждёт проверки на стенде):**
- ▶️ **3.5g панель сеанса над видео `7433028`** (см. запись 3.5g): панель — элемент раскладки, а не оверлей;
  «▲» сворачивает в полоску 8 px; чекбокс «Статистика». Web `Tests 199 passed (199)`. Локально проверено,
  **на стенде — нет:** крестик/сворачивание удалённого окна из Chrome, панель в полном экране. Не запушено.
- ▶️ **Следующее после проверки 3.5g: 3.2** (пароль хоста через PAKE, отпечаток DTLS) **или 3.4** (передача файлов)
...
**За владельцем:**
- `git push` и проверка 3.5g на стенде (крестик удалённого окна, полный экран).
