...
**Состояние на 2026-09-22 (восьмой чат: слайс 3.5 ЗАКРЫТ — проверен владельцем, включая 3.5f):**
- ▶️ **Следующее: 3.2** (пароль хоста через PAKE, отпечаток DTLS) **или 3.4** (передача файлов) — по решению
  владельца. Кандидаты помельче: D35 (сервер не знает о сеансе после реконнекта), учётки пользователя.
  Новый чат — на слайс.
