...
- 1.1–1.5 закрыты (1.5 — форма курсора и ping, живая проверка курсора — при следующем
  прогоне стека).
- 🚀 **Следующий — 1.6 (вставка в план по решению владельца 2026-09-11): размещение на VPS**:
  Dockerfile сервера, compose с coturn, ICE-серверы и TURN-креды через сигналинг, WSS у
  хоста, деплой-workflow. Сервер готов: ключи, docker-группа, секреты CI, nginx-директивы
  у владельца (см. memory `rcdesk-server-facts`).
- 1.1–1.4 закрыты. **Живой захват реального экрана проверен в Chrome** (2026-09-11, после
  ...
  доступ» у владельца.
- ✅ **Safari проверен владельцем** (2026-09-11, живой экран, «тоннель экранов»). За
  владельцем: «Универсальный доступ» для проверки ввода.
- 🌐 **Домен `rcdesk.app` куплен** (2026-09-11). Решение: на VPS отдельный пользователь
  FastPanel `rcdesk`, сайт `rcdesk.app` + LE-сертификат, A-записи `rcdesk.app` и
  `turn.rcdesk.app` (turn — без прокси Cloudflare). Порты 80/443 держит nginx FastPanel →
  вместо Caddy используем его конфиг по образцу crewtally; TURN-TLS на 5349.
