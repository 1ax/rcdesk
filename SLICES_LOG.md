...
- 1.1–1.3 закрыты: **первое живое видео в Chrome** (синтетический источник 1280x720@30,
  H264, rtt 1 мс, повторное подключение 0.3 с). Следующий — 1.4 (ввод: data channels →
  enigo), затем 1.5 (курсор). Запуск стека — `docs/dev-run.md`.
- ⚠️ **Владельцу при возвращении** (две ручные проверки, обе за минуту):
  1) Safari: открыть `http://localhost:5173`, ввести PIN — архитектор не может автоматизировать
     Safari; 2) выдать «Запись экрана» терминалу и запустить `serve` без `--synthetic`.
