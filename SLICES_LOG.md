...
- ⚪ D15: **дефолт потолка QP не выбран.** Механизм есть (`--max-qp`), причина размытости найдена
  (дельта-кадры openh264 при QP > 34; RC считает бюджет кадра по `fMaxFrameRate`, а не по реальной подаче).
  Решение — после проверки на стенде; кандидат `--max-qp 30`. Проверить и просмотр без даунскейла (окно
  Chrome меньше 1080p размывает текст независимо от кодера).
