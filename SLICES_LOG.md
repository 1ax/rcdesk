...
**Состояние на 2026-09-22 (восьмой чат: слайс 3.5 проверен владельцем; 3.5f — ждёт проверки на стенде):**
...
**За владельцем:**
- **Проверка 3.5f** (новый `rcdesk-agent.exe` из артефакта Deploy после `ba6fbb2`, страницу перезагрузить):
  в сеансе Mac → стенд в панели появился «Cmd как Ctrl» (включён); Cmd+C / Cmd+V / Cmd+Z в Блокноте работают
  как Ctrl; одиночный Cmd «Пуск» не открывает; галочку снять — Cmd снова Win. Остальное в 3.5 проверено
  2026-09-22 (см. запись 3.5). После проверки — закрыть слайс и сменить чат.
