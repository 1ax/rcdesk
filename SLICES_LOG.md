...
- **2.3b RTCP-события** (`4662595`): `SessionEvent::Remb { bitrate_bps }`, `::ReceiverReport { fraction_lost,
  ...
  от report-интерцепторов. Одно флаковое падение loopback совпало с удалением worktree на том же диске;
  пять прогонов подряд зелёные.
...
- **Фикс после первого Deploy** (`macos-latest` уронил старый тест конвейера: 15 кадров вместо ≥ 20 за
  секунду при 30 fps): пейсер с жёстким дедлайном выбрасывал пачку кадров, которой синтетика на медленной
  ВМ догоняет расписание (находка 2.1d). Заменён на token bucket с запасом 3 кадра: «должные» кадры после
  простоя проходят, устойчивая частота по-прежнему ограничена целевым fps.
...
- Гейты на закрытии (Mac): host lib **61 passed; 1 ignored**, loopback 1, proto 17, server 8 + 7,
