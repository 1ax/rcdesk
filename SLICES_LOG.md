...
**Состояние на 2026-09-21 (восьмой чат: слайс 3.5 — код в `main` и в проде, ждёт живой проверки владельца):**
- ▶️ **Слайс 3.5 переподключение и UX — пять коммитов, все запушены** (см. запись 3.5): `9f30b6b` обрыв
  ...
  `ab9cf3f` фильтр логов WebRTC-стека (D12); `81d8b28` пресеты качества «Авто/Чёткость/Плавность».
...
**За владельцем:**
- **Живая проверка 3.5** (на стенд — новый `rcdesk-agent.exe` из артефакта последнего Deploy; страницу
  https://rcdesk.app перезагрузить, бирка `build` = последний коммит):
  1. Сеанс Mac → стенд, затем перезапуск сервера (`docker compose -f ~/app/docker-compose.prod.yml restart`
     на VPS или любой push): картинка и мышь не прерываются, на экране списка — «Нет связи с сервером —
     переподключаюсь…» и возврат.
  2. Просадка сети: на стенде выдернуть кабель/отключить сеть на 5–10 с → «Связь прерывается…», потом картинка
     возвращается сама. На 30+ с → «Переподключение… (N из 5)» → новый сеанс без PIN.
  3. «Отключиться» и закрытие вкладки — хост выходит из сеанса сразу (значок в трее серый).
  4. «На весь экран» в Chrome: Cmd+Tab / Esc / Cmd+W уходят на стенд (Keyboard Lock), выход — удерживать Esc,
     подсказка 3 с. В Safari — только полный экран, выход Esc.
  5. «Качество»: «Чёткость» → оверлей `target … @ 15 fps (пресет)`, «Плавность»/«Авто» → 30 fps; выбор
     держится после перезагрузки страницы для того же устройства.
