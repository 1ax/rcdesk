...
**Состояние на 2026-09-14 (пятый чат: 2.4 закрыт, 2.4e в проде, ждёт токен бота):**
- ✅ **2.4e монитор трафика — в `main` и на VPS** (`a7b5ec0`, Deploy `34889246636` зелёный, см. запись 2.4e):
  coturn отдаёт метрики на `127.0.0.1:9641`, `~/app/traffic-monitor.sh` стоит в crontab `rcdesk` раз в
  10 минут, состояние `~/app/traffic/2026-09.state` пишется (проверено двумя ручными прогонами). Решения
  владельца: канал — Telegram-бот, на 100 % — только уведомить. **Токена ещё нет:** пока `TRAFFIC_TG_BOT_TOKEN`
  и `TRAFFIC_TG_CHAT_ID` в `~/app/.env` пусты, уведомление на пороге пропускается с записью в
  `~/app/traffic/monitor.log` и уйдёт при следующем прогоне после появления токена (метка ставится только
...
- ▶️ **Следующее: 2.5 буфер обмена** (текст в обе стороны; Chrome фоново, Safari по жесту; `arboard` на
  хосте по реестру §14). Новый чат — на слайс.
...
- Необязательно: Mac с реальным захватом после 2.4 (селектор скрыт, мышь по `CGDisplayBounds`) — стенд
  Win10 уже проверен.
- **2.4e:** создать Telegram-бота (BotFather), узнать chat_id, вписать `TRAFFIC_TG_BOT_TOKEN=` и
  `TRAFFIC_TG_CHAT_ID=` в `~/app/.env` на VPS и проверить отправку по инструкции `infra/README.md`
  («Монитор трафика», копия state в `/tmp/t` с `TRAFFIC_RELAY_BUDGET=1`).
...
**Открытые долги:**
- ⚪ D26: **хост гасит весь сеанс, если нет разрешения на ввод.** Обнаружено 2026-09-15 на живой
  проверке (Mac, реальный захват, запуск из терминала PyCharm): видеоконвейер уже поднялся
  (`starting video pipeline`, VideoToolbox 1920×1080), но `enigo` не смог инициализироваться без TCC
  «Универсальный доступ» → `failed to start session pipeline` и сессия снесена целиком, клиент видит
  чёрный экран без объяснения. Должно деградировать в режим «только просмотр» (как `--no-input`) и
  сообщать клиенту, что ввод недоступен, а не убивать видео. Обходной путь сейчас — `--no-input`.
  Примечание: TCC выдаётся ответственному приложению (родителю), для сессий из терминала PyCharm это
  PyCharm; «Запись экрана» у него есть, «Универсальный доступ» — нет.
...
- **Прод (архитектор по одобрению владельца):** после деплоя `127.0.0.1:9641/metrics` отвечает, в `# HELP`
  ...
  `relay_total=0`, `iface_last=5806768702` → второй прогон `iface_total=450`, exit 0. Токен бота владелец
  добавит утром 2026-09-15.
