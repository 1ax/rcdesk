...
- ▶️ **Следующее: 3.2e** — привязка отпечатков DTLS к ключу OPAQUE (`auth` в `Offer`/`Answer`), затем docs,
  push, деплой; проверка владельца на стенде.
...
**За владельцем:**
- **Решение владельца 2026-09-22 (вечер):** коммиты всех оставшихся под-шагов 3.2 и `git push` по итогам — за
  архитектором; владелец проверяет на следующий день. Push — только после 3.2d (3.2c без клиента ломает вход
  на хост с паролем), лучше после 3.2e.
