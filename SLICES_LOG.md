...
**Состояние на 2026-09-14 (пятый чат, слайс 2.4 закрыт кодом, ждёт стенда):**
- ✅ **Слайс 2.4 мультимонитор — в `main`, четыре коммита** (см. запись 2.4): `9f37676` контракт
  ...
  дисплея + `SendInput`/`VIRTUALDESK` на Windows. **Локально проверено** (Mac, синтетика, VT, Chrome):
  три переключения 1280×720 ↔ 1024×768 < 1 с, 30 fps держится, jb 8–10 мс, `failed` в логе нет, два PLI за
  сессию. **Стенд Win10 проверен (2026-09-14, Deploy `34876302689`):** `list-displays` →
  `65537 \\.\DISPLAY1 1920x1080@0,0 primary`; `bench` GDI+MFT стартует с `capturing display`; сессия
  Mac Chrome → стенд: селектор скрыт (один дисплей), мышь через новый `SendInput`+`VIRTUALDESK` попадает
  точно (владелец кликал сам). Замечание владельца: контекстное меню после клика по ярлыку появилось с
  задержкой ~2 с — при `app 1–3 ms` это не ввод, а видеопуть: GDI на статичном экране 1–2 fps плюс
  `rtt 190–214 ms` (снова релей, D19). Реальный захват на Mac после 2.4 не проверялся.
- ▶️ **Следующее:** сначала **D25 — вернуть crewtally** (развести имена compose-проектов), затем 2.4e монитор трафика (D22; ждёт развилок владельца — канал уведомлений и действие на
  100 %), затем **2.5 буфер обмена**. Новый чат — на слайс.
...
**За владельцем:**
- **D25 (crewtally) — следующий чат, первым делом**, см. долги.
- Необязательно: Mac с реальным захватом после 2.4 (селектор скрыт, мышь по `CGDisplayBounds`) — стенд
  Win10 уже проверен.
- Развилки 2.4e: канал уведомлений о трафике (Telegram-бот / почта / GitHub issue) и действие на 100 %
  (только уведомить / ещё и перестать выдавать TURN-креды).
...
- 🔴 D25: **crewtally.app лежит (502) с 17:20 UTC 2026-09-11 — снесена первым деплоем rcdesk.** Compose-проект
  crewtally на VPS тоже назывался `app` (`~/app` у пользователя `crewtally`: `app-api_a-1`, `app-api_b-1`,
  `app-frontend-1`, `app-worker-1`, `app-postgres-1`, `app-redis-1`), а деплой rcdesk из
  `/var/www/rcdesk/data/app` делает `docker compose up -d --remove-orphans` в проекте `app` и удалил их как
  «сирот» (лог Deploy `34626666816`). Тома `app_pg_data`/`app_redis_data` и образы crewtally целы.
  **Чинить в следующем чате, порядок:** (1) `name: rcdesk` в `infra/docker-compose.prod.yml`, пересоздать
  контейнеры rcdesk под новым проектом (старые `app-server-1`/`app-coturn-1` убрать руками, без `down -v`);
  (2) из каталога crewtally (владелец/root) её `docker compose up -d` — тома проекта `app` останутся её.
  До этого: **не делать `docker compose down -v` в rcdesk** и помнить, что каждый деплой rcdesk снесёт
  crewtally, если её поднять раньше шага (1).
- ⚪ D22: **бюджет трафика 5 ТБ/месяц исходящего** (квота VPS 10 ТБ, только исходящий; решение
  2026-09-14, ARCHITECTURE §11) не контролируется: нет помесячного счётчика на VPS и порога
  отсечки; coturn без `--max-bps`/`--total-quota`. **Запланировано как 2.4e** сразу после 2.4:
  coturn `--prometheus` на 127.0.0.1:9641 + `infra/traffic-monitor.sh` по cron `rcdesk` (раз в
  10 мин, помесячный итог, пороги 50/80/100 % от 5 ТБ, 90 % от 10 ТБ по `ens3`), см. ARCHITECTURE §11.
  Развилки владельца: канал уведомлений (Telegram-бот / почта / GitHub issue) и действие на 100 %
  (только уведомить / ещё и перестать выдавать TURN-креды). Проба VPS 2026-09-14: `curl` есть,
  crontab у `rcdesk` доступен, `vnstat` нет, интерфейс `ens3`.
