...
**Состояние на 2026-09-22 (десятый чат: слайс 3.2 — пароль хоста и защита сеанса, НАЧАТ):**
- ✅ **3.2a D35 — хост сообщает серверу о живом сеансе `da383ba`** (см. запись 3.2a): `HostRegister.session_id`,
  сервер переподвешивает известную сессию или держит «отвязанную» (busy до `Bye` хоста); хост шлёт `Bye` при
  локальном конце сеанса; клиент на `bye` внутри своего окна 15 с уходит в автопереподключение. Гейты: host
  `178 passed; 0 failed; 2 ignored`, proto `42`, server `40` + `23`, loopback `1`, web `Tests 201 passed (201)`.
  Живой проверки не требовал; push — за владельцем.
- ✅ **3.2b хранилище пароля и CLI `54627c6`** (см. запись 3.2b): `host/src/access.rs` — OPAQUE
...
- ✅ **3.2d клиент `4e131aa` + фикс троттлинга `3cda88a`** (см. записи): `@serenity-kit/opaque`, окно пароля,
  сквозная проверка против хоста на Rust локально (верный/неверный пароль, семь попыток, вход после паузы).
  Гейты: host `194 passed; 0 failed; 2 ignored`, proto `45`, server `40` + `24`, loopback `1`,
  web `Tests 219 passed (219)`, бандл 32 → 458 КБ (WASM).
- ✅ **3.2e привязка отпечатков DTLS `823e159`** (см. запись 3.2e): `auth` в `Offer`/`Answer`, HMAC от отпечатка
  из SDP на ключе OPAQUE, общий тест-вектор Rust/TS; локально: вход даёт видео, подменённый тег в `answer` хост
  отвергает. Гейты: host `206 passed; 0 failed; 2 ignored`, proto `46`, server `40` + `24`, loopback `1`,
  web `Tests 231 passed (231)`.
- ▶️ **Слайс 3.2 кодом закрыт, запушен архитектором.** За владельцем: проверка на стенде (см. «За владельцем»),
  затем 3.4 (передача файлов) в новом чате. Кандидаты помельче: D36 (лимит подбора PIN), D38, учётки.
...
**Состояние 3.5 (закрыто, 2026-09-22):**
- ✅ **3.5g панель сеанса над видео `7433028`, 3.5h полоска справа в полном экране `6878a50`** (см. записи):
  панель — элемент раскладки, а не оверлей; «▲» сворачивает в полоску 8 px (сверху в окне, справа в полном
  экране); чекбокс «Статистика». Web `Tests 199 passed (199)`. Проверено владельцем: в окне крестик
  удалённого окна достижим, в полном экране полоска справа раскрывается кликом. Push — за владельцем.
...
**За владельцем:**
- **Проверка 3.2 на стенде (после зелёного Deploy, артефакт с новым `rcdesk-host`/`rcdesk-agent`):**
  1. На стенде Win10 в cmd: `rcdesk-host.exe password set` (скрытый ввод дважды), `password status`.
  2. С Mac через https://rcdesk.app: из списка «Мои компьютеры» → окно «Пароль доступа к хосту» → неверный
     пароль («Неверный пароль»), верный → видео. Семь неверных подряд → «Повторить через 5 с», после паузы
     верный входит.
  3. Safari: то же самое (WASM `@serenity-kit/opaque` в Safari не проверялся).
  4. `password clear` → подключение без окна, как раньше. Старый хост (без 3.2) с новым прод-клиентом —
     тоже без окна.
...
**Открытые долги:**
- ⚪ D38: **после `bye` от хоста клиент показывает «Ошибка: not in session»** вместо «Сеанс завершён» — запоздавший
  ICE-кандидат уходит на сервер после закрытия сессии, ответный `Error` затирает подпись на экране PIN.
  Лечится игнором `not in session` при `sessionId === null` (замечено на негативной проверке 3.2e).
...
- ✅ D35 закрыт в 3.2a (`da383ba`): хост называет живой сеанс в `HostRegister.session_id`, сервер держит его
  занятым до `Bye` хоста; попутно хост шлёт `Bye` при любом локальном конце сеанса.
...
**Проверено владельцем 2026-09-22:** в реальном полном экране полоска справа раскрывается кликом.

## Слайс 3.2 — пароль хоста и защита сеанса (2026-09-22)

**Задача:** доступ к хосту закрывается секретом, который знает только хост (PAKE: сервер и TURN секрета
не видят и подключиться сами не могут); клиент сверяет отпечаток DTLS-сертификата хоста — защита от
подмены на сервере. Решение владельца 2026-09-22: 3.2 раньше 3.4 (файлы добавляют канал доступа, класть
его поверх сеанса без пароля хоста — удваивать поверхность); первым под-шагом — долг D35, он тоже про
регистрацию хоста и сервер.

#### 3.2a D35: хост сообщает серверу о живом сеансе при регистрации — `da383ba` (2026-09-22)

Разведка: сервер держит `HostEntry.session_id` и `SessionEntry {host_tx, client_tx}` только в памяти; при
реконнекте хоста `register_host` вытеснял старую запись и заводил новую с `session_id: None` — устройство
«в сети», второй `Join`/`ConnectDevice` проходил, и хост на `PeerJoined` гасил живой сеанс. Второй дефект
рядом: хост слал `Bye` серверу только по команде «Завершить сеанс», а при `failed`/`closed`, истечении окна
отсоединения и закрытии канала `control` — нет; сервер освобождался только по `Bye` клиента или обрыву его
WS. После D35 сервер знает о сеансе со слов хоста, и `Bye` клиента такой сеанс не найдёт — без хостового
`Bye` устройство зависло бы «занятым» до перерегистрации.

Сделано: `HostRegister.session_id: Option<String>` (`#[serde(default)]`, старый хост совместим). Сервер:
известная сессия за тем же `host_id` переподвешивается на новый `host_tx` без вытеснения клиента
(`Displaced.session = None`); неизвестная или чужая — «отвязанная»: `busy`, `Join`/`ConnectDevice` →
`host busy`, пересылать некому, освобождает `close_session_by_host` (теперь `Option<Option<Tx>>`).
`peer_tx_for_host`/`close_session_by_host`/`unregister_host` проверяют владение сессией — иначе хост,
назвавший чужой id, получил бы чужой `client_tx` (нашёл исполнитель на тесте «чужой id», принято).
Хост: `SessionSlot::current_session_id()` → `SignalingClient::connect`; `Bye` в трёх ветках локального
конца сеанса. Клиент (фикс по сверке): `bye` для текущего сеанса, пришедший пока клиент сам сидит в окне
`DISCONNECT_GRACE_MS`, — это хост, сдавшийся в той же гонке первым, а не конец сеанса: `byeOutcome` →
`handleSessionLost` → автопереподключение 3.5b вместо `teardown` (иначе новый хостовый `Bye` ломал бы 3.5b,
когда таймер хоста срабатывает раньше клиентского). Тесты: proto +2, registry +4 (a–d), сквозной WS-тест
busy → `Bye` → free, host +1 и два расширенных (`Bye` при таймауте и закрытии `control`, без дубля),
web +2. Гейты: host `178 passed; 0 failed; 2 ignored`, proto `42`, server `40` + `23`, loopback `1`,
web `Tests 201 passed (201)`. Baseline на старте отличался от журнала (proto 40, host 177 вместо 38/175):
тесты 3.5 добавлялись после записи головы. Живой проверки не требует. **Что посмотреть на стенде при
случае:** рестарт сервера посреди сеанса → в списке владельца устройство «занят», а не «в сети».

#### 3.2b Хранилище пароля и CLI — `54627c6` (2026-09-22)

Разведка (исполнитель, read-only, плюс библиотеки): единственная точка вставки проверки пароля на хосте —
начало ветки `PeerJoined`, где `start_session` первой строкой поднимает PeerConnection, захват, кодер и
ввод; отпечаток удалённого DTLS-сертификата через API `webrtc` 0.20 не достать, но оба отпечатка есть в SDP;
в дереве нет ни PAKE, ни argon2, в `web/package.json` нет runtime-зависимостей вовсе; меню трея текст не
вводит; ограничителя попыток нет ни на хосте, ни на сервере (подбор PIN на сервере — тоже, отдельный долг D36).
Выбор протокола и развилки — см. ARCHITECTURE §6.

Сделано: `host/src/access.rs` — `RcdeskCipherSuite` (Ristretto255, `TripleDh<Ristretto255, Sha512>`,
`CustomKsf` = argon2id m=65536 KiB, t=3, p=4, V0x13, соль нулевая — дословно дефолт «memory-constrained»
serenity-kit, ручной `Default`, чтобы не разъехался), `AccessRecord` (`ServerSetup` + `ServerRegistration`,
base64 URL_SAFE_NO_PAD в JSON), `AccessStore` (`access.json`, `load` толерантен к битому файлу), `register()`
(хост локально играет обе роли, `credential_identifier = "rcdesk-owner"`, идентификаторы и контекст пустые),
`login_start`/`LoginServer::login_finish` → `SessionKey` (Debug редактирован; zeroize не подключён —
`opaque_ke` его не реэкспортирует, отдельную зависимость не заводили), `validate_password` (8–128 символов,
без краевых пробелов). Атомарная запись 0600 вынесена из `device.rs` в `fsutil::write_private_atomic`,
тесты `DeviceStore` не менялись. CLI `rcdesk-host password set|clear|status`: в терминале `rpassword` с
повтором, из пайпа — одна строка stdin. Зависимости: `opaque-ke` 4.0.1 (argon2), `argon2` 0.5.3,
`sha2` 0.10.9, `rpassword` 7.5.4 и **`rand_core` 0.6 (getrandom)** сверх плана — opaque-ke обобщён над
трейтами `rand_core` 0.6, воркспейсный `rand` 0.10 несовместим, а реэкспорт `opaque_ke::rand` без фичи
`getrandom` не даёт `OsRng` (принято, записано в §14). Тесты access.rs: 7 (регистрация → вход с тем же
паролем даёт равные 64-байтные ключи; чужой пароль валится на `ClientLogin::finish`; стор round-trip, 0600,
битый JSON → `None`; правила пароля; `Default` KSF равен явным константам), 2,6 с. Ручная проверка CLI
(set из пайпа → status → clear → status) — вывод верный, файл удалён. Гейты: host
...
#### 3.2d Клиент: окно пароля и вход OPAQUE — `4e131aa`, фикс троттлинга `3cda88a` (2026-09-22)

Зависимость `@serenity-kit/opaque` 1.1.0 — первая runtime-зависимость web; WASM инлайнится в ESM-модуль
(бандл 32 → 458 КБ, gzip 172 КБ), готовность через `await opaque.ready`. `keyStretching:
"memory-constrained"` передаётся явно; `finishLogin` при неверном пароле возвращает `undefined` (проверено
в node), обёртка даёт `null`. `hostAuth.ts` — чистая машина фаз (`prompt`/`starting`/`verifying`/
`waiting-host`/`locked`) и тексты, 16 тестов; `hostAuth.opaque.test.ts` — полный цикл регистрация+вход на
настоящем пакете (сервер тоже на JS), 2 теста, доли секунды. `app.ts`: окно `#auth-dialog` поверх всего
экрана сеанса (с панелью — «Отключиться» во время входа не нужна, есть «Отмена»), имя хоста из
`joined.host_name`, поток `auth_required` → `startLogin` → `pake_start` → `pake_response` → `finishLogin` →
`pake_finish` → «Ожидание хоста…» → `offer` прячет окно; `auth_failed` с `retry_after` — отсчёт «Повторить
через N с» и заблокированное поле; «Отмена»/Esc — `bye` + «Подключение отменено». Пароль живёт только между
стартом и финишем, ключ сеанса хранится для 3.2e. Ввод сеанса не мешает: обработчики клавиатуры вешаются
на `<video>` только после открытия каналов.

**Живая проверка (архитектор, Mac, синтетика, Chrome, `password set` в реальном каталоге данных — агент не
запущен):** окно появляется сразу после `joined`; неверный пароль → «Неверный пароль» за 0,5 с; верный →
«Ожидание хоста…» → видео 1280 px через 1 с, в логе хоста `client authenticated`. Сценарий блокировки
вскрыл ошибку моего дизайна 3.2c: после пятой неудачи каждая следующая попытка (в том числе верная после
паузы) блокировалась заново, всё дольше — владелец не вошёл бы никогда. Фикс `3cda88a`: пауза ставится
перед следующей попыткой, а попытка после паузы выполняется; тест «шесть стартов → шесть ответов, седьмой
→ пауза 5 с» и «вход после истёкшей паузы сбрасывает счётчик». Повторно вживую: шесть неверных, седьмая
«Повторить через 5 с», после паузы верный пароль → видео; лог `login attempt 6, next attempt allowed in
5 s`. Гейты: host `194 passed; 0 failed; 2 ignored`, proto `45`, server `40` + `24`, loopback `1`, web
`Tests 219 passed (219)`.

#### 3.2e Привязка отпечатков DTLS к ключу OPAQUE — `823e159` (2026-09-22)

`Offer`/`Answer` получили `#[serde(default)] auth: Option<String>`; сервер пробрасывает поле. Схема на обеих
сторонах одинаковая: первая строка `a=fingerprint:` из SDP, нормализация «alg в нижнем, hex в верхнем»,
сообщение `rcdesk-dtls-v1|роль|отпечаток`, тег HMAC-SHA256 на 64-байтном `session_key`, base64 URL-safe без
набивки, сверка в постоянное время (`Mac::verify_slice` / ручное сравнение без раннего выхода). Хост
(`host/src/dtls_bind.rs`, `hmac` 0.12 добавлен хосту через workspace): в `begin_session` тег от отпечатка
своего Offer; в ветке `Answer` при наличии ключа сверка до `set_answer`, провал — `warn`, shutdown, `Bye`.
Клиент (`web/src/dtlsBind.ts`, WebCrypto): сверка Offer до `acceptOffer`, провал — `bye` + «Хост не
подтверждён: возможна подмена соединения»; тег от отпечатка своего Answer. Без пароля поле `null`/`None`,
ничего не проверяется. Общий тест-вектор (ключ 64×0x01, `offer`, `sha-256 AA:BB:CC` →
`ePsRNp-0qqZJLVlyAkuOVBJ5aAHjZxfQf27HgyY0Ncc`) в Rust и TS. Тесты: host +12 (10 в `dtls_bind`, Offer после
настоящего входа проверяется клиентским ключом против отпечатка из реального SDP webrtc-rs; Answer без
тега → `Bye`; с верным тегом — не отвергается), proto +1, web +12.

**Живая проверка (архитектор, Mac, синтетика, Chrome):** сначала позитивный сценарий провалился — «Хост не
подтверждён», в Offer не было `auth`. Причина не в коде: сервер на стенде был старой сборкой и молча
отбросил незнакомое поле при пересылке (`serde` без `deny_unknown_fields`). После пересборки сервера: вход с
паролем → Offer с тегом (43 символа), Answer с тегом, видео 1280 px. Негатив: подмена тега в `answer` через
перехват `WebSocket.prototype.send` → хост `answer failed dtls binding check, ending session`, `Bye`, клиент
на экране PIN. Попутно D38. **Грабли:** при смене контракта пересобирать все три бинарника стенда, не только
хост. Гейты: host `206 passed; 0 failed; 2 ignored`, proto `46`, server `40` + `24`, loopback `1`,
web `Tests 231 passed (231)`, бандл 459 КБ.
